...
async function main() {
  ...
  const hasListProjects = tools.tools?.some(tool => tool.name === 'list_projects');
  if (!hasListProjects) {
    throw new Error('Angular MCP server did not register the list_projects tool.');
  }
  ...
  if (!Array.isArray(workspaces) || workspaces.length === 0) {
    throw new Error('Angular MCP server could not find an Angular workspace from this root.');
  }
